// Data for the Multimap component
const multimapData = [
  { method: 'insert()', syntax: 'mm.insert({k, v})', params: 'pair', output: 'iterator', complexity: 'O(log n)', desc: 'Inserts key-value pair (allows duplicates)' },
  { method: 'erase()', syntax: 'mm.erase(k)', params: 'key', output: 'size_t', complexity: 'O(log n + k)', desc: 'Removes all elements with key' },
  { method: 'find()', syntax: 'mm.find(k)', params: 'key', output: 'iterator', complexity: 'O(log n)', desc: 'Finds element, returns iterator' },
  { method: 'count()', syntax: 'mm.count(k)', params: 'key', output: 'size_t', complexity: 'O(log n + k)', desc: 'Returns number of elements with key' },
  { method: 'size()', syntax: 'mm.size()', params: '—', output: 'size_t', complexity: 'O(1)', desc: 'Returns number of elements' },
  { method: 'clear()', syntax: 'mm.clear()', params: '—', output: 'void', complexity: 'O(n)', desc: 'Removes all elements' }
];

const tableHeaders = ['Method', 'Syntax', 'Input / Parameters', 'Output / Return', 'Time Complexity', 'Description'];

export default function MultimapTable() {
  return (
    <section 
      id="multimap" // ID for sidebar anchor
    ...
    >
      ...
      <div className="p-6 md:p-8 border-b border-gray-200 dark:border-[#333]">
        <h2 className="text-2xl md:text-3xl font-semibold text-gray-900 dark:text-white">
          <span className="font-mono text-red-500 mr-3">C.4</span>
          <span className="font-inter">Multimap (Ordered, Allows Duplicate Keys)</span>
        </h2>
        <p className="mt-2 text-gray-600 dark:text-gray-400 font-light">
          An associative container that stores sorted key-value pairs, allowing duplicate keys.
        </p>
      </div>
